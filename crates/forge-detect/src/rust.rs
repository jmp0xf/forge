...
use forge_core::{
    CommandSource, CommandSpec, Confidence, CoverageDimension, Intent, Inventory, Mutability,
    NetworkIntent, PathKind, ProjectKind, ProjectModel, ProjectUnit, Provenance, RelativePathError,
    RepoRelativePath, ToolchainInfo, UnitEdge,
};
...
pub fn detect_rust_project(
    ...
    for manifest in manifests {
        let (mut completion, workspace) = probe_manifest(context, &manifest);
    ...
    fallback_manifests.retain(|manifest| {
    ...
    let command_plan_fragments =
        rust_default_plan_fragments(&units).map_err(RustDetectionError::InvalidCommandPlan)?;
...
fn probe_manifest(
    ...
    let spec = cargo_metadata_exec_spec(manifest, context.metadata_timeout);
...
/// Builds one additive ordered Rust plan fragment for each supported command intent.
...
fn rust_plan_for_intent(
    ...
    let phases: &[RustCommandPhase] = match intent {
        ...
        Intent::Check => &[RustCommandPhase::FormatCheck, RustCommandPhase::Check],
        Intent::Test => &[
...
fn rust_command(intent: Intent, phase: RustCommandPhase, scope: &RustCommandScope) -> CommandSpec {
    ...
    if scope.workspace && matches!(phase, RustCommandPhase::Check | RustCommandPhase::Test) {
        args.push("--workspace");
    }
    match phase {
        RustCommandPhase::Check => args.push("--all-targets"),
    ...
    command.coverage = match phase {
        ...
        RustCommandPhase::Check => BTreeSet::from([CoverageDimension::Compile]),
...
mod tests {
    ...
    fn multiple_independent_packages_remain_separate_and_ordered() -> Result<(), Box<dyn Error>> {
        ...
        let check = result
            ...
            .find(|plan| plan.intent() == Intent::Check)
            .ok_or("missing check plan")?;
        assert_eq!(check.commands().len(), 4);
        assert_eq!(check.commands()[0].cwd.as_path(), Path::new("a"));
        assert_eq!(check.commands()[2].cwd.as_path(), Path::new("b"));
        assert!(
            check
                .commands()
                .iter()
