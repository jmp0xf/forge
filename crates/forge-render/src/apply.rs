    ...
    H: Hasher + ?Sized,
{
    ...
    for edit in prepared {
        let current = filesystem
            .read_confined(repository_root, &edit.path)
            .map_err(|source| {
                ApplyError::io(
...
where
    ...
    H: Hasher + ?Sized,
{
    match (edit.kind, edit.expected_preimage.is_some()) {
        (FileEditKind::Create, false) | (FileEditKind::ReplaceManagedBlock, true) => {}
        _ => {
            return Err(ApplyError::plain(
                ApplyErrorKind::InvalidEdit,
                Some(edit.path.clone()),
                "edit kind contradicts expected preimage presence",
                report.clone(),
            ));
        }
    }
    ...
    let merged = merge_markdown_block(existing.as_deref(), &block, hasher, edit.force)
        .map_err(|source| ApplyError::managed_block(edit.path.clone(), source, report.clone()))?;
    let expected_action = match edit.kind {
        FileEditKind::Create => merged.action == MergeAction::Create,
        FileEditKind::ReplaceManagedBlock => {
            matches!(merged.action, MergeAction::Append | MergeAction::Replace)
        }
    };
    if !expected_action {
        return Err(ApplyError::plain(
            ...
            Some(edit.path.clone()),
            format!(
                "recomputed managed-block action {:?} does not match edit kind {:?}; no-op decisions belong to planning",
                merged.action, edit.kind
            ),
            report.clone(),
        ));
    }
...
mod tests {
    ...
    use crate::managed_block::{ManagedBlock, merge_markdown_block};
    use crate::plan::{
    ...
    fn edit(
        ...
        let merged = merge_markdown_block(
            existing,
            &ManagedBlock {
                id: desired.kind.id(),
                body: &desired.body,
